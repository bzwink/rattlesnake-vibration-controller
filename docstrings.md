...
----------
test_environment
    Verifies that a mock environment subclass can be initialized with
...
Continuously reads command and data pairs from the environment command
queue until the shutdown event is set or a mapped command returns a
truthy halt flag. Each received command is looked up in ``command_map``
and the corresponding function is called with the received data.

If an undefined command is received, the environment logs the issue and
continues running. If a mapped command raises an exception, the traceback
is logged, an error message is sent to the GUI update queue, and the
environment continues running unless shutdown is otherwise requested.
...
Subclasses must implement this method to safely stop control activity.
This should include any operations needed to avoid abrupt output changes,
protect test equipment, stop background activity, and clear environment
