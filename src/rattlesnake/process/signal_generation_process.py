...
class SignalGenerationMetadata:
    ...
    def __init__(
        ...
        self.new_signal_sample_threshold = (
            self.samples_per_write
            if new_signal_sample_threshold is None
            else new_signal_sample_threshold
        )
...
class SignalGenerationProcess(AbstractMessageProcess):
    ...
    def initialize_parameters(self, data: SignalGenerationMetadata):
        ...
        self.output_transformation_matrix = (
            None
            if data.output_transformation_matrix is None
            else np.linalg.pinv(data.output_transformation_matrix)
        )
    ...
    def generate_signals(self, data):
        ...
        if (
            ...
            and self.signal_remainder is not None
            and self.signal_remainder.shape[-1] > 0
        ):
            ...
            last_run = (self.shutdown_flag and self.current_test_level == 0.0) or (
                self.done_generating and self.signal_remainder.shape[-1] == 0
            )
    ...
    def shutdown(self):
        ...
        self.environment_command_queue.put(
            self.process_name, (SignalGenerationCommands.SHUTDOWN_ACHIEVED, None)
        )
        self.startup = True
        self.shutdown_flag = False
        self.done_generating = False
